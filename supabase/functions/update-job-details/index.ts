...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (assignedTo) {
      const { data: serviceProfile } = await adminClient.from("profiles").select("id,role,is_active").eq("id", assignedTo).single();
      if (!serviceProfile || serviceProfile.role !== "service" || !serviceProfile.is_active) return json({ error: "Seçilen servis personeli geçerli değil." }, 400);
    }
    const { error } = await adminClient.from("jobs").update({ customer_name: customerName, customer_phone: customerPhone, description, scheduled_at: scheduledAt, priority, assigned_to: assignedTo }).eq("id", jobId);
