...
export default function Home() {
  ...
  async function syncLiveData() {
    ...
    const { data: auth } = await supabase.auth.getUser()
    if (!auth.user) return
    const [{ data: liveJobs }, { data: liveNotices }] = await Promise.all([
      supabase
        .from('jobs')
    ...
    if (liveJobs) setJobs(liveJobs as Job[])
    if (liveNotices) setNotices(liveNotices as Notice[])
  }
  ...
  useEffect(() => {
    ...
    const startFallbackSync = () => {
      ...
      timer = setInterval(() => {
        if (document.visibilityState === 'visible') void syncLiveData()
      }, 2000)
    }
    ...
    const channel = supabase.channel(`team-live-${crypto.randomUUID()}`)
      ...
      .subscribe(status => {
        if (status === 'SUBSCRIBED') void syncLiveData()
        if (status === 'TIMED_OUT' || status === 'CHANNEL_ERROR') void syncLiveData()
      })
